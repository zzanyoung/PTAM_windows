...
pair<SE2<>, double> SmallBlurryImage::IteratePosRelToTarget(SmallBlurryImage& other, int nIterations)
{
	...
	for (int it = 0; it < nIterations; it++)
	{
		...
		//----------------------- 2023.08.25 @kchanyou, CVD::transform, CVD::transform#sample() 직접구현----------------------- 
		const int w = imWarped.size().x, h = imWarped.size().y, iw = mimTemplate.size().x, ih = mimTemplate.size().y;
		const TooN::Vector<2> across = se2XForm.get_rotation().get_matrix().T()[0];
		const TooN::Vector<2> down = se2XForm.get_rotation().get_matrix().T()[1];

		const TooN::Vector<2> p0 = se2XForm.get_translation() - se2XForm.get_rotation().get_matrix() * v2Zero;
		...
		if (min_x >= 0 && min_y >= 0 && max_x < iw - 1 && max_y < ih - 1)
		{
			...
			for (int i = 0; i < h; ++i, p += carriage_return)
				for (int j = 0; j < w; ++j, p += across)
				{
					int lx = (int)p[0];
					int ly = (int)p[1];
					int w = mimTemplate.size().x;
					const float* base = mimTemplate[ly] + lx;
					float a = base[0];
					float b = base[1];
					float c = base[w];
					float d = base[w + 1];
					...
					imWarped[i][j] = (float)(x * (y * (e - c + d) - e) + y * (c - a) + a);
				}
			resultReturn = 0;
		}
		else
